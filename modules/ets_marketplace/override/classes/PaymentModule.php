...
class PaymentModule extends PaymentModuleCore
{
    ...
    {
        if(Module::isEnabled('ets_marketplace') && ($ets_marketplace = Module::getInstanceByName('ets_marketplace')) && $ets_marketplace->checkListProductSeller($productList))
        {
        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (isset($order->id)) {
                    if (!$secure_key) {
                        if(version_compare(_PS_VERSION_, '1.7', '>='))
                            $message .= Ets_mp_defines::displayText('','br').$this->trans('Warning: the secure key is empty, check your payment account before validation', array(), 'Admin.Payment.Notification');
                        else
                            $message .= Ets_mp_defines::displayText('','br').Tools::displayError('Warning: the secure key is empty, check your payment account before validation');
                    }
                            ...
                            foreach ($customized_datas[$product['id_product']][$product['id_product_attribute']][$order->id_address_delivery] as $customization) {
                                ...
                                if (isset($customization['datas'][Product::CUSTOMIZE_TEXTFIELD])) {
                                    foreach ($customization['datas'][Product::CUSTOMIZE_TEXTFIELD] as $text) {
                                        $customization_text .= Ets_mp_defines::displayText($text['name'],'strong','').$text['value'].Ets_mp_defines::displayText('','br');
                                    }
                                }
                                if (isset($customization['datas'][Product::CUSTOMIZE_FILE])) {
                                    if(version_compare(_PS_VERSION_, '1.7', '>='))
                                        $customization_text .= $this->trans('%d image(s)', array(count($customization['datas'][Product::CUSTOMIZE_FILE])), 'Admin.Payment.Notification') . Ets_mp_defines::displayText('','br');
                                    else
                                        $customization_text .= sprintf(Tools::displayError('%d image(s)'), count($customization['datas'][Product::CUSTOMIZE_FILE])).Ets_mp_defines::displayText('','br');
                                }
                    ...
                    if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id) {
                        ...
                        $data = array(
                            ...
                            '{delivery_block_html}' => $this->_getFormatedAddress($delivery, Ets_mp_defines::displayText('','br'), array(
                                'firstname' => Ets_mp_defines::displayText('%s','span','bold'),
                                'lastname' => Ets_mp_defines::displayText('%s','span','bold'),
                            )),
                            '{invoice_block_html}' => $this->_getFormatedAddress($invoice, Ets_mp_defines::displayText('','br'), array(
                                'firstname' => Ets_mp_defines::displayText('%s','span','bold'),
                                'lastname' => Ets_mp_defines::displayText('%s','span','bold'),
                            )),
