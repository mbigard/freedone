...
function ets_mp_searchLocationsNear(center)
{
	...
	ets_mp_downloadUrl(searchUrl2, function(data) {
    	...
    	for (var i = 0; i < markerNodes.length; i++)
    	{
    		...
    		$('table#stores-table').find('tbody').append('<tr ><td class="num">'+parseInt(i + 1)+'</td><td class="name">'+(has_store_picture ? '<img src="'+has_store_picture+'" alt="" />' : '')+'<span><a href="'+link_shop+'">'+name+'</a></span></td><td class="address">'+address+(phone !== '' ? '<br/>'+translation_4+' '+phone : '')+'</td></tr>');
    		$('#stores-table').show();
    	}
