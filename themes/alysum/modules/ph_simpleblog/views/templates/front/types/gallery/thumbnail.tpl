...
{if isset($post.banner) && Configuration::get('PH_BLOG_DISPLAY_THUMBNAIL')}
	<div class="post-thumbnail">
		<a {if isset($post.gallery) && sizeof($post.gallery)}href="{$gallery_dir|escape:'html':'UTF-8'}{$post.gallery.0.image|escape:'html':'UTF-8'}.jpg" data-fancybox-group="post-gallery-slideshow-{$post.id_simpleblog_post|intval}" class="post-gallery-link"{else}href="{$post.url|escape:'html':'UTF-8'}"{/if} title="{l s='Permalink link to' d='Modules.Phsimpleblog.Shop'} {$post.title|escape:'html':'UTF-8'}">
