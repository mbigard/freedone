...
{capture name=path}
	<a href="{$link->getModuleLink('ph_simpleblog', 'list')|escape:'html':'UTF-8'}" title="{l s='Back to blog homepage' d='Modules.Phsimpleblog.Shop'}">
	...
	{/if}
	<a href="{$post->category_url|escape:'html':'UTF-8'}" title="{l s='Visit category' d='Modules.Phsimpleblog.Shop'}">
...
{if isset($smarty.get.confirmation)}
	...
	{if $smarty.get.confirmation == 1}
		{l s='Your comment was sucessfully added.' d='Modules.Phsimpleblog.Shop'}	
	{else}
		{l s='Your comment was sucessfully added but it will be visible after moderator approval.' d='Modules.Phsimpleblog.Shop'}	
	{/if}
...
<div itemscope="itemscope" itemtype="http://schema.org/Blog" itemprop="mainContentOfPage">
	<div class="ph_simpleblog simpleblog-single {if !empty($post->featured_image)}with-cover{else}without-cover{/if} simpleblog-single-{$post->id_simpleblog_post|intval}" itemscope="itemscope" itemtype="http://schema.org/BlogPosting" itemprop="blogPost">
		...
		<div class="post-meta-info">
			...
			{if $post->tags && Configuration::get('PH_BLOG_DISPLAY_TAGS') && isset($post->tags_list)}
				<span class="post-tags clear">
					{l s='Tags:' d='Modules.Phsimpleblog.Shop'} 
			...
			{if Configuration::get('PH_BLOG_DISPLAY_LIKES')}
				<span class="post-likes">
					<a href="#" data-guest="{$cookie->id_guest|intval}" data-post="{$post->id_simpleblog_post|intval}" class="simpleblog-like-button">
						...
						<span>{$post->likes|intval}</span> {l s='likes'  d='Modules.Phsimpleblog.Shop'}
					</a>
				</span>
			{/if}
			...
			<span class="post-views">
				<i class="fa fa-eye"></i> {$post->views|escape:'html':'UTF-8'} {l s='views'  d='Modules.Phsimpleblog.Shop'}
			</span>
			{/if}

			{if $allow_comments eq true && Configuration::get('PH_BLOG_COMMENTS_SYSTEM') == 'native'}
			<span class="post-comments">
				<i class="fa fa-comments"></i> {$post->comments|escape:'html':'UTF-8'} {l s='comments'  d='Modules.Phsimpleblog.Shop'}
			</span>
			{/if}
		</div><!-- .post-meta-info -->
		...
		<div class="post-gallery">
			...
			<a rel="post-gallery-{$post->id_simpleblog_post|intval}" class="fancybox" href="{$gallery_dir|escape:'html':'UTF-8'}{$image.image|escape:'html':'UTF-8'}.jpg" title="{l s='View full' d='Modules.Phsimpleblog.Shop'}"><img src="{$gallery_dir|escape:'html':'UTF-8'}{$image.image|escape:'html':'UTF-8'}.jpg" class="img-responsive" /></a>
			{/foreach}
		</div><!-- .post-gallery -->
...
{if Configuration::get('PH_BLOG_FB_INIT') || false}
<script>
