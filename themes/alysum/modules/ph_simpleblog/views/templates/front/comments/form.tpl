...
<form class="std clearfix" action="{$post->url|escape:'html':'UTF-8'}" method="post">
	<fieldset>
		<div class="box">
			<h3 class="page-heading bottom-indent">{l s='New comment' d='Modules.Phsimpleblog.Shop'}</h3>
			<div class="form-group">
				<label for="customer_name">{l s='Your name' d='Modules.Phsimpleblog.Shop'}</label>
			...
			<div class="form-group">
				<label for="comment_content">{l s='Your comment' d='Modules.Phsimpleblog.Shop'}</label>
		...
		<p class="cart_navigation required submit clearfix">
			...
			<button type="submit" class="button btn btn-default button-medium" name="submitNewComment" id="submitNewComment">
				<span>
					{l s='Add new comment' d='Modules.Phsimpleblog.Shop'}
					<i class="fa fa-chevron-right right"></i>
				</span>
			</button>
			{else}
			<input type="submit" class="button" name="submitNewComment" value="{l s='Add new comment' d='Modules.Phsimpleblog.Shop'}" />
			{/if}
		</p>
	</fieldset>
</form>
{else}
	<div class="warning alert alert-warning">
		<a href="{$link->getPageLink('authentication', true, null, ['back' => $post->url])|escape:'html':'UTF-8'}">{l s='Only registered and logged customers can add comments' d='Modules.Phsimpleblog.Shop'}</a>
	</div><!-- .warning -->
{/if}
