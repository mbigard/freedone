...
{block name='step_content'}
    ...
    {if !$payment_options && isset($amp_po)}
        {assign var="payment_options" value=$amp_po} {* get payment methods from AMP controller*}
    {/if}
        ...
        {foreach from=$payment_options item="module_options"}
            ...
            <div id="{$option.id}-container" class="payment-option clearfix">
                ...
                <span class="custom-radio float-xs-left">
                    <input class="ps-shown-by-js {if $option.binary} binary {/if}" id="{$option.id}"
                        data-module-name="{$option.module_name}" name="payment-option" type="radio" required
                        {if ($selected_payment_option == $option.id || (isset($is_free) && $is_free)) || ($payment_options|@count === 1 && $module_options|@count === 1)}
                        checked {/if}>
                    <span></span>
                </span>
                    ...
                    <li>
                        <div class="dib">
                            <span class="custom-checkbox">
                                <input id="conditions_to_approve[{$condition_name}]" name="conditions_to_approve[{$condition_name}]"
                                    required type="checkbox" value="1" class="ps-shown-by-js" on="change:selector-tos.toggle">
                                <span><svg class="svgic">
                                    <use href="{_THEME_IMG_DIR_}lib.svg#done"></use>
                                </svg></span>
                            </span>
                        </div>
